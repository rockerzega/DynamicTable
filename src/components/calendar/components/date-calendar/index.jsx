...
const DateCalendar = ({
  ...
  const clickDay = useCallback(
    (day, month = date.month() + 1, year = date.year()) => {
      ...
      function chosePeriod(start, end) {
        ...
        console.log('antes de ejecutar la funcion del context');
        changeDatepickerValue(
          {
        ...
        console.log('se cambio las fechas')
        hideDatepicker();
      }
      ...
      if ((!period.start && !period.end) || (period.start && period.end)) {
        ...
        console.log('period end ', period)
        console.log(newStart, newEnd)
      }
      console.log('period ', period)
      console.log(newStart, newEnd)
      if (!(newEnd && newStart) || showFooter) {
        console.log('en el tercer if')
        changePeriod({
          start: newStart,
          end: newEnd
        });
  ...
  return (
    <div className="w-full" style={{minWidth: '296px', width: '296px'}}>
      <div
        className="flex items-center border border-gray-300 rounded-md px-2 space-x-1.5 py-1.5"
      >
        {!showMonths && !showYears && (
          <div className="flex-none" id="cmp-1">
